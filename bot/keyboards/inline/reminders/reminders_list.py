...
def get_reminders_list_inline_markup(curr_list: str, is_edit: bool = False, curr_page: int = 1,
    ...
    if max_page > 1:
        markup.row(
            InlineKeyboardButton('⬅', callback_data=f'reminders:{curr_list}:{curr_list}:{action}:{curr_page - 1 if curr_page > 1 else curr_page}:{search_filter}'),
            InlineKeyboardButton(f'{curr_page}/{max_page}', callback_data='reminders:none'),
            InlineKeyboardButton('➡', callback_data=f'reminders:{curr_list}:{curr_list}:{action}:{curr_page + 1 if curr_page < max_page else curr_page}:{search_filter}'))
    markup.row(InlineKeyboardButton(tiles[0]['text'], callback_data=f'{tiles[0]["callback"]}'),
               InlineKeyboardButton(_('🔄 Обновить'), callback_data=f'reminders:{curr_list}:{curr_list}:{action}:{curr_page}:{search_filter}'),
               InlineKeyboardButton(tiles[1]['text'], callback_data=f'{tiles[1]["callback"]}'))
    markup.row(InlineKeyboardButton(_('🔎 Поиск') if search_filter == ":" else _("❌ Сбросить фильтр"),
                                    callback_data='search' if search_filter == ":" else f'reminders:{curr_list}:{curr_list}:{action}:max:{":"}'),
               InlineKeyboardButton(_('✏ Редактировать') if not is_edit else _('👀 Просмотр'),
                                    callback_data=f'reminders:{curr_list}:{curr_list}:edit:{curr_page}:{search_filter}' if not is_edit else f'reminders:{curr_list}:{curr_list}:view:{curr_page}:{search_filter}'))
    return markup
