...
def get_reminders_list_inline_markup(hidden_button: str, is_edit: bool = False, curr_page: int = 1,
    ...
    if max_page > 1:
        markup.row(
            InlineKeyboardButton('⬅', callback_data=f'reminders:{curr_page - 1 if curr_page > 1 else curr_page}'),
            InlineKeyboardButton(f'{curr_page}/{max_page}', callback_data='reminders:none'),
            InlineKeyboardButton('➡',
                                 callback_data=f'reminders:{curr_page + 1 if curr_page < max_page else curr_page}'))
    markup.row(InlineKeyboardButton(tiles[0]['text'], callback_data=f'{tiles[0]["callback"]}'),
               InlineKeyboardButton(_('🔄 Обновить'), callback_data=f'reminders:{curr_page}'),
