...
async def actual_reminders_list_callback(callback_query: CallbackQuery, callback_data: dict, session: AsyncSession,
    ...
    page = int(callback_data['curr_page']) if callback_data['list'] == callback_data['curr_list'] and callback_data['curr_page'] != 'max' else 0

    if match_filter == ":":
        text, max_page = await get_list(function_list[callback_data['list']], callback_data['action'] == 'edit', session, user.id, page)
    else:
        text, max_page = await get_list(function_list[callback_data['list']], callback_data['action'] == 'edit', session, user.id, page, column, _filter)

    page = int(callback_data['curr_page']) if callback_data['list'] == callback_data['curr_list'] and callback_data['curr_page'] != 'max' else max_page
