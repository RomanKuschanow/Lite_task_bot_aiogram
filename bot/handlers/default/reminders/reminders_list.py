...
async def reminders_list(message: Message, session: AsyncSession, user: User):
    ...
    await message.answer(text, reply_markup=get_reminders_inline_markup("all", curr_page=max_page, max_page=max_page))
    await message.delete()


@dp.callback_query_handler(list_callback.filter())
@rate_limit(2)
async def actual_reminders_list_callback(callback_query: CallbackQuery, callback_data: dict, session: AsyncSession, user: User):
    ...
    if await get_list(function_list[callback_data['list']], callback_data['action'] == 'edit', session, user.id, max_page) != "":
        text = await get_list(function_list[callback_data['list']], callback_data['action'] == 'edit', session, user.id, max_page)

    await callback_query.message.edit_text(text, reply_markup=get_reminders_inline_markup(callback_data['list'], callback_data['action'] == 'edit', curr_page=max_page, max_page=max_page))


async def get_list(function, is_edit, *parametrs) ->str:
    text = ""

    deep_link = "http://t.me/Lite_task_bot?start=edit_task_1"
    ...
    for reminder in reminders if max_page == 1 else reminders[-25 * range(1, max_page + 1)[-parametrs[2]]:][:25]:
        text += f'{"✅" if reminder.is_reminded else "❌"} {reminder.text}: {reminder.date.strftime("%d.%m.%Y %H:%M")}' + (f'<a href="{deep_link}{reminder.id}">✏</a>\n' if is_edit else '\n')
