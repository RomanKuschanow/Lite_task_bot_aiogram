...
async def page_select(callback_query: CallbackQuery, callback_data: dict, session: AsyncSession, user: User):
    ...
    if re.search('reminders:(.+):.+', keyboard[1][0]['callback_data'])[1] == "old" and \
       re.search('reminders:(.+):.+', keyboard[1][1]['callback_data'])[1] == "actual":
    ...
    text = await get_list(function, re.search('reminders:.+:(.+)', keyboard[1][0]['callback_data'])[1] == "edit", session, user.id, page)


    max_page = int(len(await function(session, user.id)) / 25) + 1
    await callback_query.message.edit_text(text, reply_markup=get_reminders_inline_markup(hidden_button, re.search('reminders:.+:(.+)', keyboard[1][0]['callback_data'])[1] == "edit", curr_page=page, max_page=max_page))
